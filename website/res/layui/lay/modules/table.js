...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function(exports){
  ...
  ,thisTable = function(){
    ...
    return {
      ...
      ,config: options
      ,renderData: function (res, curr, count, sort) {
          that.renderData.call(that, res, curr, count, sort);
      }, refresh: function() {
          that.fullSize();
          that.scrollPatch();
      }
  ...
  Class.prototype.renderData = function(res, curr, count, sort){
    ...
    ,render = function(){
      ...
      that.layFixLeft.show().find('tbody').html(trs_fixed.join(''));
      that.layFixRight.show().find('tbody').html(trs_fixed_r.join(''));
    ...
    if(data.length === 0){
      that.renderForm();
      that.layFixed.hide().find('tbody').html('');
      that.layMain.find('tbody').html('');
      that.layMain.find('.' + NONE).remove();
      that.layTool.children().empty();
  ...
  Class.prototype.fullSize = function(){
    ...
    //tbody区域高度
    bodyHeight = parseFloat(height) - parseFloat(that.layHeader.height() || 24) - 1;  
    if(options.page){
